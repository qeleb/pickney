...
async function assembleUserState(user) {
    ...
    let items = await db.collection(`items`).find({ owner: user.id }).toArray();
    let comments = await db.collection(`comments`).find({ item: { $in: items.map(item => item.id) } }).toArray();
    let users = [
        await db.collection(`users`).findOne({ id: user.id }),
        ...await db.collection(`users`).find({ id: { $in: [...items, comments].map(x => x.owner) } }).toArray()
    ];
